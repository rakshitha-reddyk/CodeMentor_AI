...
import SkillProgressChartCard from "@/components/SkillProgressChartCard";
import RecentActivityCard from "@/components/RecentActivityCard";
import GoalTrackerCard from "@/components/GoalTrackerCard";
import LeaderboardCard from "@/components/LeaderboardCard";
import AIMentorSuggestionsCard from "@/components/AIMentorSuggestionsCard";
...
const Dashboard = () => {
  ...
  // Local state for offline-first localStorage progress
  ...
  // Load progress from localStorage on mount and whenever component is focused
  useEffect(() => {
    const loadLocalProgress = () => {
      try {
        const analyticsData = progressStore.getAnalytics();
        const progressData = progressStore.getAllProgress();
    ...
    // Also reload when window becomes visible (user returns to tab)
  ...
  // Seed lessons on mount
  useEffect(() => {
    seedLessons().catch((err) => console.error("Failed to seed lessons:", err));
  }, []);
  ...
  const stats = backendUtils.calculateStats(
    localAnalytics || analytics,
    localProgress.length > 0 ? localProgress : progress || [],
  );
    ...
    <div className="w-full flex-1 fade-in">
            ...
            <div className="flex-1">
              ...
              <p className="text-3xl font-bold text-purple-400">
                {backendUtils.formatTimeSpent(
                  localAnalytics?.total_time_spent ||
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <LearningStreakCard />
          <SkillProgressChartCard />
          <RecentActivityCard />
          <GoalTrackerCard />
          <LeaderboardCard />
          <AIMentorSuggestionsCard />
        </div>
      </div>

      {/* Continue Learning and Study Planner */}
        ...
        <div>
          ...
          {lessonsLoading ? (
            ...
            <div className="rounded-xl border border-border/50 bg-surface-elevated/30 p-12 text-center backdrop-blur">
              ...
              <h4 className="text-lg font-semibold mb-2">No Active Lessons</h4>
              <p className="text-muted-foreground mb-6">
                Start exploring our curated lessons to begin your journey
              </p>
              <Button
                onClick={() => navigate("/learn")}
                className="bg-gradient-to-r from-brand-primary to-brand-primary/80"
              >
